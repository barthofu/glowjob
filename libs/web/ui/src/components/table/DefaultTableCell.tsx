...
export function EditableTableCell<TData>(props: DefaultTableCellProps<TData>) {
    ...
    const initialValue = props.children || props.getValue()
    const meta = props.column.columnDef.meta // not typed on purpose
    ...
    const onUpdate = (overrideValue?: any) => {
        ...
        let newValue = overrideValue ?? value
        console.log('onUpdate', newValue, valueHasChanged)
        if (!valueHasChanged && value === initialValue && meta?.type !== 'checkbox') return
        ...
        // cast
        if (meta.type === 'number') newValue = Number(newValue)
        else if (meta.type === 'boolean') newValue = Boolean(newValue)
    ...
    useEffect(() => {
        setValueHasChanged(value !== initialValue)
    }, [value])
