	...
	plop.setGenerator('component', {
		...
		prompts: [
			...
			},
			{
				...
				choices: ['shared', 'modules', 'layouts'],
			},
			{
				type: 'list',
				name: 'category',
				...
				choices: (answers) => {
					const categories = fs
						.readdirSync(
							`${basePath.web}/src/components/${answers.type}`
						)
						.filter((file) =>
							fs
								.statSync(
									`${basePath.web}/src/components/${answers.type}/${file}`
								)
								.isDirectory()
						)
		...
		actions: [
			...
			},
			{
				...
				template:
					'export * from \'./{{#ifNotEquals type "layouts"}}{{#ifNotEquals category "No Category" }}{{category}}/{{/ifNotEquals}}{{/ifNotEquals}}{{pascalCase name}}\'',
			},
		],
	})
}
